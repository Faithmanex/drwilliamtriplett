...
const Books: React.FC = () => {
  ...
  const handleConfirmPurchase = async (e: React.FormEvent) => {
    ...
    // Get download URL from env
    const downloadUrls: Record<string, string> = {
      'harbor-hopes': import.meta.env.VITE_DOWNLOAD_URL_HARBOR_HOPES || '',
      'ethical-algorithm': import.meta.env.VITE_DOWNLOAD_URL_ETHICAL_ALGORITHM || '',
      'sacred-signals': import.meta.env.VITE_DOWNLOAD_URL_SACRED_SIGNALS || '',
      'fractured-foundations': import.meta.env.VITE_DOWNLOAD_URL_FRACTURED_FOUNDATIONS || '',
    };
    const downloadUrl = downloadUrls[selectedBook.id] || '#';
      ...
      <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
        ...
        <div style="padding: 40px 30px;">
          ...
          <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 24px; margin-bottom: 30px;">
            <div style="font-family: 'Georgia', serif; font-size: 20px; font-weight: bold; color: #1e293b; margin-bottom: 5px;">${selectedBook.title}</div>
            <div style="color: #64748b; font-style: italic; font-size: 14px; margin-bottom: 20px;">${selectedBook.subtitle}</div>
    ...
    try {
      const response = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: `Your Resource: ${selectedBook.title}`,
          html: emailHtml,
        }),
      });
